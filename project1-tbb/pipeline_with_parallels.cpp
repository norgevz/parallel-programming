...
public:
    ...
    void* operator()(void *item){
        ...
        // Detect faces
        task->face_cascade.detectMultiScale(task->frame_gray, task->faces, 1.1, 2, 0 | CASCADE_SCALE_IMAGE, Size(30, 30));
        return task;
    }
};
...
public:
    stage3() : tbb::filter(tbb::filter::parallel){}
    void* operator()(void* item){
        task_t *task = static_cast <task_t*> (item);
...
public:
    stage4() : tbb::filter(tbb::filter::serial_in_order){}
    void* operator()(void* item){
        task_t *task = static_cast <task_t*> (item);
